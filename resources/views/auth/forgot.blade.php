...
@section('form-auth')
    <form action="{{ route('auth.forgotpassword.post') }}" method="POST" onsubmit="return false;" id="formEnterUsername">
        ...
        <div class="form-auth-login">
            <h3 class="text-uppercase text-center font-weight-bold">{{ __("auth.forgot_password") }}</h3>
            <div class="form-group">
                <label for="enter_username">{{ __('auth.enter_username') }}</label>
                <input type="text" class="form-control" name="username" id="enter_username" placeholder="username" required autocomplete="off">
            </div>
    ...
    <form action="{{ route('auth.confirmotp.post') }}" method="POST" onsubmit="return false;" id="formConfirmOTP" style="display: none">
        @csrf
        <div class="form-auth-login">
            <h3 class="text-uppercase text-center font-weight-bold">{{ __("auth.otp_header") }}</h3>
            <div class="form-group mb-2">
                <label for="enter_otp_code">{{ __('auth.enter_otp_code') }}</label>
                <input type="text" class="form-control" name="otp_code" id="enter_otp_code" placeholder="123456" autocomplete="off">
            </div>
            <div class="form-group text-center">
    ...
    <form action="{{ route('auth.editpasswordforgot.post') }}" method="POST" onsubmit="return false;" id="formEditPassword" style="display: none">
        @csrf
        <div class="form-auth-login">
            <h3 class="text-uppercase text-center font-weight-bold">{{ __("auth.new_password") }}</h3>
            <div class="form-group">
